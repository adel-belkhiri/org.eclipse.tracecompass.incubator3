...
public class VhostAvailableDescriptorsDataProvider extends AbstractTreeCommonXDataProvider<@NonNull DpdkVhostAnalysisModule, @NonNull TmfTreeDataModel> {

    /**
     * Title used to create XY models for the {@link VhostAvailableDescriptorsDataProvider}.
     */
     ...
     * Extension point ID.
     */
    public static final String ID = "org.eclipse.tracecompass.incubator.dpdk.vhost.data.provider"; //$NON-NLS-1$
    ...
    /**
     * Create an instance of {@link VhostAvailableDescriptorsDataProvider}. Returns a null instance if
     * the analysis module is not found.
     *
     * @param trace
     *            A trace on which we are interested to fetch a model
     * @return A {@link VhostAvailableDescriptorsDataProvider} instance. If analysis module is not
     *         found, it returns null
     */
    public static VhostAvailableDescriptorsDataProvider create(ITmfTrace trace) {
        ...
        if (module != null) {
            module.schedule();
            return new VhostAvailableDescriptorsDataProvider(trace, module);
        }
        return null;
    }
     ...
     * Constructor
     */
    private VhostAvailableDescriptorsDataProvider(ITmfTrace trace, DpdkVhostAnalysisModule module) {
        super(trace, module);
    }
    ...
    protected TmfTreeModel<@NonNull TmfTreeDataModel> getTree(ITmfStateSystem ss, Map<String, Object> parameters,
        ...
        for (Integer devQuark : ss.getQuarks(IDpdkVhostModelAttributes.DEVICES, "*")) {
            ...
            if(vidsQuark != ITmfStateSystem.INVALID_ATTRIBUTE) {
    ...
    private List<VirtQueueBuilder> initBuilders(ITmfStateSystem ss, SelectionTimeQueryFilter filter) {
        ...
        for (Entry<Long, Integer> entry : getSelectedEntries(filter).entrySet()) {
            ...
            int metricQuark = ss.optQuarkRelative(quark, IDpdkVhostModelAttributes.AVAIL_DESCR);

            if (metricQuark != ITmfStateSystem.INVALID_ATTRIBUTE) { /*ss.getAttributeName(quark).equals(IDpdkVhostModelAttributes.AVAIL_DESCR)) */
                long id = entry.getKey();
                String queueName = getQuarkValue(ss, quark);

                int queuesType = ss.getParentAttributeQuark(quark);
                String queueTypeName = getQuarkValue(ss, queuesType);

                int vidQuark = ss.getParentAttributeQuark(queuesType);
                int connfdQuark = ss.optQuarkRelative(vidQuark, IDpdkVhostModelAttributes.CONNFD);
                String connfdName = "fd-"+ getQuarkValue(ss, connfdQuark);
                ...
                builders.add(new VirtQueueBuilder(id, quark, name, length));
            }
        }
        return builders;
    }
    ...
    public static double extractCount(int queueQuark, List<ITmfStateInterval> states, ITmfStateSystem ss) {
        ...
        int metricQuark = ss.optQuarkRelative(queueQuark, IDpdkVhostModelAttributes.AVAIL_DESCR);
