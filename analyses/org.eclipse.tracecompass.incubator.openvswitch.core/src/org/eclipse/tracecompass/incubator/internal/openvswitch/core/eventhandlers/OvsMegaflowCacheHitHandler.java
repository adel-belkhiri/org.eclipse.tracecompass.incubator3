 ...
 *
 */
public class OvsMegaflowCacheHitHandler extends OpenvSwitchEventHandler {
     ...
     */
    public OvsMegaflowCacheHitHandler(IOpenvSwitchAnalysisEventLayout layout, OvsFlowsStateProvider stateProvider) {
        super(layout);
    ...
    public void handleEvent(@NonNull ITmfStateSystemBuilder ss, @NonNull ITmfEvent event) throws AttributeNotFoundException {
        ...
        if (dpUfid == null) {
            throw new IllegalArgumentException("ovs_megaflow_cache_hit event does not have expected fields"); //$NON-NLS-1$ ;
        }

        /* First update some statistics */
        fOvsFlowsSP.incrementCacheHitNumbers(ts, false /*isEmcCacheHit*/);

        DatapathModel dp = fOvsFlowsSP.searchDatapathByDpUfid(dpUfid);
        if(dp != null) {
