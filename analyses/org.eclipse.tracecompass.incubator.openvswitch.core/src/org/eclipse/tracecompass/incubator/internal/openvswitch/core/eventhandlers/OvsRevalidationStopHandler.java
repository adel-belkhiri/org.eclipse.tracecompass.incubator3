 ...
 *
 */
public class OvsRevalidationStopHandler extends OpenvSwitchEventHandler {

    /**
     * @param layout :
     * @param stateProvider :
     */
    public OvsRevalidationStopHandler(@NonNull IOpenvSwitchAnalysisEventLayout layout, OvsRevalidationStateProvider stateProvider) {
        super(layout);
    }
    ...
    public void handleEvent(@NonNull ITmfStateSystemBuilder ss, @NonNull ITmfEvent event) throws AttributeNotFoundException {
        ...
        if (revalidator_id == null || dump_seq == null || dp_name == null) {
            throw new IllegalArgumentException("ovs_dpif:sweep_start event does not have expected fields"); //$NON-NLS-1$ ;
        }
        ...
        ss.modifyAttribute(ts, "wait blocked", revalidatorQuark); //$NON-NLS-1$
    }
